...
def preprocess_path_for_deform_task(gt_path, distorted_path):
    ...
    for distorted_image in distorted_image_list:
        ...
        if not os.path.isfile(gt_image):
            print(gt_image)
            gt_image = os.path.join(gt_path, image+'.jpg')
            if not os.path.isfile(gt_image):
                print('still not found, ',gt_image)
                continue
            else:
                print('found,',gt_image)
...
class LPIPS():
    ...
    def calculate_mask_lpips(self, path_1, path_2, batch_size=64, verbose=False, sort=True):
        ...
        imgs_1=[]
        imgs_2=[]
        bonesLst = '/dataset/ljw/market/market-annotation-test.csv'
...
if __name__ == "__main__":
    ...
    lpips_score = lpips.calculate_from_disk(distorated_list, gt_list, sort=False, batch_size=64)

    # market dataset default calculate Mask-LPIPS
    print('calculate mask LPIPS...')
