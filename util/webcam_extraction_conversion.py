...
def crop_and_reshape_preds(preds, pad, out_shape=224):
...
def crop_and_reshape_img(img, preds, pad, out_shape=224):
    minX, maxX, minY, maxY = get_borders(preds)
...
def filter_preds(previous, preds):
    
    return (np.array(previous) + np.array(preds))/2

def generate_landmarks(cap, device, pad,previous_preds = None):
...
output: x the camera output, g_y the corresponding landmark"""
    ...
    #Get webcam image
    # fa = face_alignment.FaceAlignment(face_alignment.LandmarksType._2D,face_detector='dlib', flip_input=False, device='cuda:0')
    ...
    while(no_pic == True):
        ...
        if ret:
            ...
            for i in range(len(frames_list)):
                ...
                preds = filter_preds(previous_preds, preds)                    
    ...
    if ret:
        ...
        frame_mark = frame_mark.permute(0,1,4,2,3).to(device) #K,2,3,256,256
    ...
    return x,g_y,ret, previous_preds
