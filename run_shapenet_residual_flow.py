...
from dataset.shapenet_dataset import  make_dataset
...
def test(opt):
    ...
    for i_batch, batch_data in enumerate(pbar, start=0):
        ...
        test_name = name + '_target_' + th + '_' + tv + '_source_'
        for k in range(len(ref_ids)):
            sh = ref_ids[k][0].split('_')[1]
            sv = ref_ids[k][0].split('_')[2]
            test_name += sh + '_' + sv
            if k != len(ref_ids)-1:
                test_name += '-'
