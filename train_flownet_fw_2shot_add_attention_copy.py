...
experiment_name = 'FuseG_v8_2shot_maskmulflow_mask_{0}_soft_{1}_enc_{2}_btn_{3}_lr1e4-1007'.format(use_mask, soft_mask,n_enc_dec_layers, n_bottleneck_layers)
...
def train():
    ...
    for epoch in range(epochCurrent, num_epochs):
        ...
        for i_batch, (ref_xs, ref_ys, g_x, g_y, idx) in enumerate(pbar, start=0):
            ...
            mask_11 = mask_1 * flow_1 # mask * flow means 
            mask_22 = mask_2 * flow_2
            mask = F.softmax(torch.cat((mask_11 / torch.sum(mask_11), mask_22 / torch.sum(mask_22)), dim=1), dim=1) # pixel wise sum to 1
