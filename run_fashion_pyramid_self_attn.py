...
from model.pyramid_flow_generator_with_occlu_attn import FlowGenerator, AppearanceDecoder, AppearanceEncoder, PoseAwareSelfAttentionDecoder
...
def init_generator(opt, path_to_chkpt, path_to_flow_chkpt=None):
    ...
    if not opt.use_pose_decoder:
        ...
        GD = PoseAwareSelfAttentionDecoder(structure_nc=structure_nc, n_decode_layers=3, output_nc=3, flow_layers=flow_layers, ngf=64, max_nc=256, norm_type=opt.norm_type,
        ...
        for i_batch, batch_data in enumerate(pbar, start=0):
            ...
            if opt.use_pose_decoder:
                if opt.use_input_mask:
                    ...
                    x_hat, attn_norm = GD(g_y, xfs, flows)
                    attn_norm_trans = []
                    for k in range(0, opt.K):
                        temp = []
