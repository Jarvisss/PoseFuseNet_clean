...
experiment_name = 'FuseG_v8_2shot_add_direct_pretrain{0}_mask_{1}_soft_{2}_enc_{3}_btn_{4}_lr1e4-1007'.format(direct_pretrain, use_mask, soft_mask,n_enc_dec_layers, n_bottleneck_layers)
...
with torch.no_grad():
    ...
    for video_name in video_names:
        ...
        for gt_id in tqdm(range(5, total_ids, 5)):
            ...
            ref_x2 = load_image(os.path.join(path_to_test_imgs, ref2_name+'.png')).unsqueeze(0).to(device) # [B,3,256,256]
            ref_y2 = load_skeleton(os.path.join(path_to_test_img_kps, ref2_name+'.json')).unsqueeze(0).to(device)

            ref_x1 = load_image(os.path.join(path_to_test_imgs, ref1_name+'.png')).unsqueeze(0).to(device) # [B,3,256,256]
            ...
            '''Direct branch'''
            # [B*2, 3, 256, 256]
            ref_xs_compact = torch.cat((ref_x1, ref_x2), dim=0)
            # print(ref_xs_compact.shape)
            # [B*2, 20, 256, 256]
            ref_ys_compact = torch.cat((ref_y1, ref_y2), dim=0)
            # print(ref_ys_compact.shape)
            
            e_vectors = direct_E(ref_xs_compact, ref_ys_compact) #BK * 512 * 1
            e_vectors = e_vectors.view(-1, 2, 512, 1) # [B, K, 512 ,1]
            ...
            visualize_f1_warp_masked, visualize_f2_warp_masked, merged_f = visualize_feature_group([xf1_warped_masked,xf2_warped_masked,xf_merge], DISPLAY_BATCH, img_shape) 
