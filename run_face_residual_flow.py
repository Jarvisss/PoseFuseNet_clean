...
from model.pyramid_flow_generator_with_occlu_attn import FlowGenerator, AppearanceDecoder, AppearanceEncoder, PoseAwareResidualFlowDecoder, PoseAwareAppearanceDecoder
from model.discriminator import ResDiscriminator
from model.blocks import warp_flow,_freeze,_unfreeze

from util.vis_util import get_pyramid_visualize_result, tensor2im, to_image
from torch.optim.lr_scheduler import MultiStepLR, ReduceLROnPlateau

from dataset.face_dataset import  make_dataset
...
def get_parser():
    ...
    parser.add_argument('--n_res_block', type=int, default=1)
    ...
    parser.add_argument('--use_res_flow', action='store_true')
...
def init_generator(opt, path_to_chkpt, path_to_flow_chkpt=None):
    ...
    if not opt.use_pose_decoder:
        ...
        GD = PoseAwareAppearanceDecoder(structure_nc=structure_nc, n_decode_layers=3, output_nc=3, flow_layers=flow_layers, ngf=64, max_nc=256, norm_type=opt.norm_type,
    ...
    for epoch in range(epochCurrent, opt.epochs):
        ...
        for i_batch, batch_data in enumerate(pbar, start=0):
            ...
            for k in range(0, opt.K):
                flow_ks, _, _ = GF(ref_xs[k], ref_ys[k], g_y) # 32, 64
                flow_k = flow_ks[1]
                # print(flow_k.shape)
        ...
        for i_batch, batch_data in enumerate(pbar, start=0):
            ...
            if opt.use_pose_decoder:
                ...
                if opt.use_input_mask:
                    ...
                    if opt.use_res_flow:
                        x_hat, out_flows, out_attns = GD(g_y, xfs, flows, masks, attn_norm_trans)
                    else:
    ...
    for i_batch, batch_data in enumerate(pbar, start=0):
        ...
        full_img,simp_img, out_img = get_pyramid_visualize_result(opt, \
            ref_xs=ref_xs, ref_ys=ref_ys,ref_ps=ref_ps, gx=g_x, x_hat=x_hat, gy=g_y,\
                gp=gp,gp_hat=gp_hat,flows=flows, attns_normed=attn_norm_trans, occlusions=masks, ref_features=xfs, g_features=gf, refined_flows=out_flows, refined_attns=out_attns)
        # full_img,simp_img, out_img = get_pyramid_visualize_result(opt, \
        #     ref_xs=ref_xs, ref_ys=ref_ys,ref_ps=ref_ps, gx=g_x, x_hat=x_hat, gy=g_y,\
...
def test_video(opt, driving_video_path,ref_id=789):
    import cv2

    os.environ["CUDA_VISIBLE_DEVICES"] = str(opt.gpu)
    experiment_name = opt.test_id
    from util.webcam_extraction_conversion import generate_landmarks
    '''Create Model'''
    image_nc = 3
    structure_nc = 3
    flow_layers = [2,3]
    ...
    path_to_chkpt_G = path_to_ckpt_dir + '{0}.tar'.format(opt.test_ckpt_name) 
    test_result_dir = '/home/ljw/playground/poseFuseNet/test_result/{0}/{1}/{2}_shot_video/'.format(experiment_name, opt.test_ckpt_name, opt.K)
    test_result_eval_dir = '/home/ljw/playground/poseFuseNet/test_result/{0}/{1}/{2}_shot_eval/'.format(experiment_name, opt.test_ckpt_name, opt.K)
    if not os.path.isdir(test_result_dir):
        os.makedirs(test_result_dir)
    '''Loading from past checkpoint_G'''
    ...
    # driving_video_path = '/home/ljw/playground/Realistic-Neural-Talking-Head-Models/examples/fine_tuning/test_video.mp4'
    drive_name = driving_video_path.split('/')[-1].split('.')[0]
    print(drive_name)
    video_dir = os.path.join(test_result_dir, drive_name)
    ...
    for i_batch, batch_data in enumerate(pbar, start=0):
        ...
        ret = True
        i_frame = 0
        size = (224*(opt.K+3),224)

        video = cv2.VideoWriter(os.path.join(test_result_dir,'%s_%d.mp4'%(drive_name, i_batch)),cv2.VideoWriter_fourcc(*'DIVX'), fps, size)
        video_frames_dir = os.path.join(video_dir, str(i_batch))
        ...
        with torch.no_grad():
            # while i_frame<10:
            while ret:
                g_x, g_y, ret, previous_preds = generate_landmarks(cap=cap, device=device, pad=50, previous_preds=previous_preds)
                flows, masks, attns,  xfs = [], [], [], []
                if ret:
                    g_y = g_y.unsqueeze(0)/255.0 * 2 -1 # 1,3,224,224
                    g_x = g_x.unsqueeze(0)/255.0 * 2 -1 # 1,3,224,224
                    for k in range(0, opt.K):
                    ...
                    # x_hat = G(g_y, e_hat) # 1,3,224,224
                    plt.clf()
                    out1 = x_hat[0].permute(1,2,0) # 224,224,3
                    out1 = to_image(out1.to(cpu).numpy())

                    out2 = g_x[0].permute(1,2,0)
                    out2 = to_image(out2.to(cpu).numpy())
                    out3 = g_y[0].permute(1,2,0)
                    out3 = to_image(out3.to(cpu).numpy())

                    inputs = [to_image(ref_xs[k][0].permute(1,2,0).to(cpu).numpy()) for k in range(opt.K)]
                    inputs = np.concatenate(inputs, axis=1)
                    inputs = cv2.cvtColor(inputs, cv2.COLOR_BGR2RGB)
                    # out2 = cv2.cvtColor(out2, cv2.COLOR_BGR2RGB)
                    out3 = cv2.cvtColor(out3, cv2.COLOR_BGR2RGB)
                    out1 = cv2.cvtColor(out1, cv2.COLOR_BGR2RGB)
                    img = np.concatenate((inputs, out2, out3, out1), axis=1)
                    img = img.astype('uint8')
                    video.write(img)
                    cv2.imwrite(os.path.join(video_frames_dir, '%04d.jpg'%i_frame), img)

                    i_frame+=1
                    print(i_frame,'/',n_frames)
        cap.release()
        video.release()
    ...
    if opt.phase == 'train':
        ...
        with torch.no_grad():
            if opt.pretrain_flow:
                ...
                test_video(opt, opt.driving_video_path)
            else:
                test(opt)
