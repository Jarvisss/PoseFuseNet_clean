...
from model.pyramid_flow_generator_with_occlu_attn import FlowGeneratorWithMask
...
def init_generator(opt, path_to_chkpt):
    ...
    if opt.parallel:
        GF = nn.DataParallel(FlowGeneratorWithMask(inc=GF_inc,n_layers=opt.n_enc, norm_type=opt.norm_type, use_spectral_norm=opt.use_spectral_G, mask_use_sigmoid=opt.mask_sigmoid).to(device)) # dx + dx + dy = 3 + 20 + 20
        ...
        GF = FlowGeneratorWithMask(image_nc=3, structure_nc=21, n_layers=5, flow_layers=[2,3], ngf=32, max_nc=256, norm_type=opt.norm_type,use_spectral_norm=opt.use_spectral_G).to(device)
...
def train_flow_net(opt, exp_name):
    ...
    criterionMask = nn.BCEWithLogitsLoss()
