...
from model.pyramid_flow_generator_with_occlu_attn import  AppearanceEncoder, UnifyGenerator
...
from dataset.fashion_dataset import FashionDataset, make_dataset
...
def save_generator(parallel, epoch, lossesG, GE, GD, i_batch, optimizerG,path_to_chkpt_G):
...
def init_generator(opt, path_to_chkpt, path_to_flow_chkpt=None):
    ...
    G_global = UnifyGenerator(structure_nc=structure_nc, n_encode_layers=5, n_decode_layers=3, output_nc=3, flow_layers=flow_layers, flow_ngf=32, target_ngf=64, max_nc=256,
    norm_type=opt.norm_type, activation=opt.activation, use_spectral_norm=opt.use_spectral_G,use_self_attention=opt.use_self_attention, align_corners=opt.align_corner, use_resample=opt.G_use_resample)
    
    if opt.parallel:
        G_global = nn.DataParallel(G_global) # dx + dx + dy = 3 + 20 + 20
        GE = nn.DataParallel(GE)

    GE = GE.to(device)
    G_global = G_global.to(device)

    optimizerG = optim.Adam(params = list(G_global.parameters()) + list(GE.parameters()) ,
                            lr=opt.lr,
                            amsgrad=False)
    if opt.use_scheduler:
        lr_scheduler = ReduceLROnPlateau(optimizerG, 'min', factor=np.sqrt(0.1), patience=5, min_lr=5e-7)
    ...
    if not os.path.isfile(path_to_chkpt):
        ...
        if not opt.flow_onfly:
            if path_to_flow_chkpt is not None:
                ...
                if opt.parallel:
                    G_global.module.load_state_dict(checkpoint_flow['GF_state_dict'], strict=False)
                else:
                    G_global.load_state_dict(checkpoint_flow['GF_state_dict'], strict=False)

            else:
                print('Please specify the pretrained flow model path')
        
        save_generator(opt.parallel, 0, [],GE, G_global, 0, optimizerG, path_to_chkpt)
        print('...Done')

    return GE, G_global, optimizerG
...
def train(opt, exp_name):
    ...
    '''Create Model'''
    GE, GD, optimizerG = init_generator(opt, path_to_chkpt_G, path_to_chkpt_flow)
    
    '''Loading from past checkpoint_G'''
    # load_chkpt_G = path_to_ckpt_dir + 'epoch_0_batch_2000_G.tar'
    # load_chkpt_D = path_to_ckpt_dir + 'epoch_0_batch_2000_D.tar'
    ...
    for epoch in range(epochCurrent, opt.epochs):
        ...
        for i_batch, batch_data in enumerate(pbar, start=0):
            ...
            x_hat, flows, res_flows, attn_norm_trans, masks = GD(ref_xs, ref_ys, xfs, g_y, sim_maps=None)
            # out_flows = flows.copy()
            out_flows = [[0]*len(flows[0]) for k in range(len(flows)) ]
            for k in range(opt.K):
                for i in range(len(flows[k])):
                    out_flows[k][i] = flows[k][i] + res_flows[k][i]
            if opt.use_adv:
            ...
            if opt.use_correctness:
                ...
                for k in range(opt.K):
                    ...
                    # loss_correct += criterionL1(xfs_warp[k], g_xf)
                loss_correct = loss_correct/opt.K * opt.lambda_correctness / 2
            ...
            if i_batch % opt.img_save_freq == 0:
                ...
                if opt.use_sim_attn_loss or opt.use_attn_reg:
                    ...
                    for i in range(opt.K):
                        final_img[256*6:256*7, 256*(opt.K+i):256*(opt.K+i+1)] = sims_vis[i]
                        final_img[256*7:256*8, 256*(opt.K+i):256*(opt.K+i+1)] = masked_sims_vis[i]
            ...
            if i_batch % opt.model_save_freq == 0:
                ...
                save_generator(opt.parallel, epoch, lossesG, GE, GD, i_batch, optimizerG, path_to_save_G)
                save_discriminator(opt.parallel, D, optimizerD, path_to_save_D) 
        
        save_generator(opt.parallel, epoch+1, lossesG, GE, GD, i_batch, optimizerG, path_to_chkpt_G)
...
def test(opt):
    ...
    GE = AppearanceEncoder(n_layers=3, inc=3, ngf=64, max_nc=256, norm_type=opt.norm_type, activation=opt.activation, use_spectral_norm=opt.use_spectral_G)
    GD = UnifyGenerator(structure_nc=structure_nc, n_encode_layers=5, n_decode_layers=3, output_nc=3, flow_layers=flow_layers, flow_ngf=32, target_ngf=64, max_nc=256,
    norm_type=opt.norm_type, activation=opt.activation, use_spectral_norm=opt.use_spectral_G,use_self_attention=opt.use_self_attention, align_corners=opt.align_corner, use_resample=opt.G_use_resample)
    
    if opt.parallel:
        GD = nn.DataParallel(GD) # dx + dx + dy = 3 + 20 + 20
        GE = nn.DataParallel(GE)
        ...
        if not opt.use_input_mask: 
            full_img,simp_img, out_img = get_pyramid_visualize_result(opt, \
                        ref_xs=ref_xs, ref_ys=ref_ys,ref_ps=None, gx=g_x, x_hat=x_hat, gy=g_y,\
                            gp=None,gp_hat=None,flows=flows, attns_normed=attn_norm_trans, occlusions=masks, ref_features=xfs, g_features=gf, refined_flows=out_flows)
        else:
            full_img,simp_img, out_img = get_pyramid_visualize_result(opt, \
                        ref_xs=ref_xs, ref_ys=ref_ys,ref_ps=ref_ms, gx=g_x, x_hat=x_hat, gy=g_y,\
                            gp=g_m,gp_hat=mask_merged,flows=flows, attns_normed=attn_norm_trans, occlusions=masks, ref_features=xfs, g_features=gf, refined_flows=out_flows)
