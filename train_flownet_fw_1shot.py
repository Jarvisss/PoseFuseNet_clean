...
def train():
    ...
    for epoch in range(epochCurrent, num_epochs):
        ...
        if epoch % save_freq == 0:
            ...
            outf1_warp = F.interpolate(xf1_warped, size=out.shape[0:2], mode='bilinear',align_corners=False)[DISPLAY_BATCH].permute(1,2,0) * 255.0
            outf1_warp_masked = F.interpolate(xf1_warped_masked, size=out.shape[0:2], mode='bilinear',align_corners=False)[DISPLAY_BATCH].permute(1,2,0) * 255.0
            outf1 = F.interpolate(xf1, size=out.shape[0:2], mode='bilinear',align_corners=False)[DISPLAY_BATCH].permute(1,2,0) * 255.0
            ...
            ref1 = torch.cat((ref_x1, ref_y1), dim =0)
            gt = torch.cat((g_x, g_y), dim=0)
            out1_col = torch.cat((flowimg1, visualize_f1_warp),dim=0)
            outmask_col = torch.cat((maskimg_1, visualize_f1_warp_masked),dim=0)
            out_col = torch.cat((out, visualize_f1),dim=0)

            if use_mask:
                final_img = torch.cat((ref1, out1_col, outmask_col, out_col, gt), dim=1)
            else:
                final_img = torch.cat((ref1, out1_col, out_col, gt), dim=1)
