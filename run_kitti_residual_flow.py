...
from model.pyramid_flow_generator_with_occlu_attn import ShapeNetFlowGenerator, AppearanceDecoder, AppearanceEncoder, PoseAwareResidualFlowDecoderShapenet
from model.discriminator import ResDiscriminator
from model.blocks import warp_flow,_freeze,_unfreeze

from util.vis_util import get_pyramid_visualize_result, tensor2im
from torch.optim.lr_scheduler import ReduceLROnPlateau

from dataset.scene_dataset import  make_dataset
...
def init_flowgenerator(opt, path_to_chkpt):
    image_nc = 3
    # structure_nc = 18+3 # one hot pose (18 for h and 3 for v)
    structure_nc = 6 # 6 DOF pose vector
    flow_layers = [2,3]
    
    GF = ShapeNetFlowGenerator(inc=image_nc, struct_nc=structure_nc, n_layers=5, flow_layers= flow_layers, ngf=32, max_nc=256, norm_type=opt.norm_type, activation=opt.activation, use_spectral_norm=opt.use_spectral_G)
    if opt.parallel:
...
def init_generator(opt, path_to_chkpt, path_to_flow_chkpt=None):
    ...
    '''
    image_nc = 3
    # structure_nc = 18 + 3 # one hot vectors for angle 18 + 3 
    structure_nc = 6 # one hot vectors for angle 18 + 3 
    ...
    if not opt.use_pose_decoder:
        ...
        GD = PoseAwareResidualFlowDecoderShapenet(structure_nc=structure_nc, n_decode_layers=3, output_nc=3, flow_layers=flow_layers, ngf=64, max_nc=256, norm_type=opt.norm_type,
    ...
    for epoch in range(epochCurrent, opt.epochs):
        ...
        for i_batch, batch_data in enumerate(pbar, start=0):
            ...
            for i in range(len(ref_xs)):
                ref_xs[i] = ref_xs[i].to(device).float()
                ref_ys[i] = ref_ys[i].to(device).float()

            g_x = g_x.to(device).float() # [B, 3, 256, 256]
            g_y = g_y.to(device).float() # [B, 18, 1, 1]
...
def train(opt, exp_name):
    ...
    for epoch in range(epochCurrent, opt.epochs):
        ...
        for i_batch, batch_data in enumerate(pbar, start=0):
            ...
            g_x = g_x.to(device).float() # [B, 3, 256, 256]
            g_y = g_y.to(device).float() # [B, 20, 256, 256]
...
def test(opt):
    ...
    for i_batch, batch_data in enumerate(pbar, start=0):
        ...
        g_x = batch_data['g_x']
        g_y = batch_data['g_y']
        ref_ids = batch_data['ref_ids']
        g_id = batch_data['g_id'][0]
        assert(len(ref_xs)==len(ref_ys))
        assert(len(ref_xs)==opt.K)
        for i in range(len(ref_xs)):
            ref_xs[i] = ref_xs[i].to(device).float()
        ...
        vid = g_id.split('_')[0]
        frame = g_id.split('_')[1]
        

        test_name = vid + '_target_' + frame + '_source_'
        for k in range(len(ref_ids)):
            frame = ref_ids[k][0].split('_')[1]
            test_name += frame
            if k != len(ref_ids)-1:
                test_name += '-'
