...
from dataset.market_dataset import make_dataset
...
def init_generator(opt, path_to_chkpt, path_to_flow_chkpt=None):
    ...
    if not opt.use_pose_decoder:
        ...
        GD = PoseAwareResidualFlowDecoder(structure_nc=structure_nc, n_decode_layers=3, output_nc=3, flow_layers=flow_layers, ngf=64, max_nc=256, norm_type=opt.norm_type,
...
def train(opt, exp_name):
    ...
    for epoch in range(epochCurrent, opt.epochs):
        ...
        for i_batch, batch_data in enumerate(pbar, start=0):
            ...
            # # residual_flow constrain
            # if opt.use_res_flow_l1:
            #     loss_resflow_l1 = 0
            #     for k in range(opt.K):
